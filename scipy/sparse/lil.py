...
class lil_matrix(spmatrix, IndexMixin):
    ...
    def tocsr(self, copy=False):
        ...
        M, N = self.shape
        lengths = np.fromiter(
            map(len, self.rows), dtype=get_index_dtype(maxval=N), count=M
        )
        nnz = lengths.sum()
        ...
        if M == 0:
            ...
            indices = np.fromiter(
                (x for y in self.rows for x in y), dtype=idx_dtype, count=nnz
            )
            data = np.fromiter(
                (x for y in self.data for x in y), dtype=self.dtype, count=nnz
            )
        # init csr matrix
        from .csr import csr_matrix
