...
class TestBinomP(object):
    ...
    def test_alternatives(self):
        res = stats.binom_test(51, 235, p=1.0 / 6, alternative="less")
        assert_almost_equal(res, 0.982022657605858)

        res = stats.binom_test(51, 235, p=1.0 / 6, alternative="greater")
        assert_almost_equal(res, 0.02654424571169085)

        res = stats.binom_test(51, 235, p=1.0 / 6, alternative="two-sided")
...
class TestPpccMax(object):
    ...
    def test_brack(self):
        ...
        # it is accurate up to 16 decimals
        assert_almost_equal(
            stats.ppcc_max(x, brack=(0, 1)), -0.71215366521264145, decimal=5
        )

        # On Python 2.6 the result is accurate to 5 decimals. On Python >= 2.7
        # it is accurate up to 16 decimals
        assert_almost_equal(
            stats.ppcc_max(x, brack=(-2, 2)), -0.71215366521264145, decimal=5
        )
