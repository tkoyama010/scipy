...
class beta_gen(rv_continuous):
    ...
    def _stats(self, a, b):
        ...
        g2 = 6.0 * (
            a ** 3 + a ** 2 * (1 - 2 * b) + b ** 2 * (1 + b) - 2 * a * b * (2 + b)
        )
        g2 /= a * b * (a + b + 2) * (a + b + 3)
        return mn, var, g1, g2
...
class levy_stable_gen(rv_continuous):
    ...
    def _rvs(self, alpha, beta):
        ...
        TH = uniform.rvs(
            loc=-np.pi / 2.0, scale=np.pi, size=sz, random_state=self._random_state
        )
    ...
    def _cdf(self, x, alpha, beta):
        ...
        for pair in uniq_param_pairs:
            ...
            if (
                ...
                warnings.warn(
                    u"FFT method is considered experimental for "
...
class logistic_gen(rv_continuous):
    ...
    def _logpdf(self, x):
        return -x - 2.0 * sc.log1p(np.exp(-x))
...
class nct_gen(rv_continuous):
    ...
    def _pdf(self, x, df, nc):
        ...
        ncx2 = nc * nc * x2
        fac1 = n + x2
        trm1 = n / 2.0 * np.log(n) + sc.gammaln(n + 1)
        trm1 -= (
            n * np.log(2)
            + nc * nc / 2.0
            + (n / 2.0) * np.log(fac1)
    ...
    def _stats(self, df, nc, moments="mv"):
        ...
        gfac = sc.gamma(df / 2.0 - 0.5) / sc.gamma(df / 2.0)
...
class pearson3_gen(rv_continuous):
    ...
    def _stats(self, skew):
        _, _, _, _, _, beta, alpha, zeta = self._preprocess([1], skew)
        m = zeta + alpha / beta
        v = alpha / (beta ** 2)
    ...
    def _rvs(self, skew):
        ...
        ans[invmask] = self._random_state.standard_gamma(alpha, nbig) / beta + zeta
...
class rice_gen(rv_continuous):
    ...
    def _rvs(self, b):
        ...
        t = b / np.sqrt(2) + self._random_state.standard_normal(size=(2,) + self._size)
...
def _truncnorm_logsf(x, a, b):
    ...
    if b < 0:
        with np.errstate(divide="ignore"):
            nla = _norm_logcdf(a)
            nlb = _norm_logcdf(b)
            return np.log1p(-np.exp(_norm_logcdf(x) - nlb)) - np.log1p(
                -np.exp(nla - nlb)
            )
    with np.errstate(divide="ignore"):
        sla = _norm_logsf(a)
...
def _truncnorm_ppf(q, a, b):
    ...
    if q <= 0.5:
        ...
        args = (a, b, np.log(q))
        ret = optimize.zeros.brentq(
            _f_cdf, a, b, args=args, maxiter=TRUNCNORM_MAX_BRENT_ITERS
        )
        return ret
    else:
        ...
        ret = optimize.zeros.brentq(
            _f_sf, a, b, args=args, maxiter=TRUNCNORM_MAX_BRENT_ITERS
        )
        return ret
