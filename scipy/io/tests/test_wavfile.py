...
def test_read_1():
    for mmap in [False, True]:
        rate, data = wavfile.read(datafile("test-44100Hz-le-1ch-4bytes.wav"), mmap=mmap)
...
def test_read_2():
    for mmap in [False, True]:
        rate, data = wavfile.read(datafile("test-8000Hz-le-2ch-1byteu.wav"), mmap=mmap)
...
def test_read_3():
    for mmap in [False, True]:
        rate, data = wavfile.read(
            datafile("test-44100Hz-2ch-32bit-float-le.wav"), mmap=mmap
        )
        assert_equal(rate, 44100)
...
def test_read_4():
    for mmap in [False, True]:
        with suppress_warnings() as sup:
            ...
            rate, data = wavfile.read(
                datafile("test-48000Hz-2ch-64bit-float-le-wavex.wav"), mmap=mmap
            )
...
def test_read_5():
    for mmap in [False, True]:
        rate, data = wavfile.read(
            datafile("test-44100Hz-2ch-32bit-float-be.wav"), mmap=mmap
        )
        assert_equal(rate, 44100)
        assert_(np.issubdtype(data.dtype, np.float32))
        assert_(
...
def test_read_early_eof():
    for mmap in [False, True]:
        with open(
            datafile("test-44100Hz-le-1ch-4bytes-early-eof-no-data.wav"), "rb"
        ) as fp:
