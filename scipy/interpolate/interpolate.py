...
def interpn(points, values, xi, method="linear", bounds_error=True, fill_value=np.nan):
    ...
    if ndim > 2 and method == "splinef2d":
        raise ValueError(
            "The method spline2fd can only be used for " "2-dimensional input data"
        )
    ...
    if len(points) != ndim and method == "splinef2d":
        raise ValueError(
            "The method spline2fd can only be used for "
    ...
    for i, p in enumerate(xi.T):
        if bounds_error and not np.logical_and(
            np.all(grid[i][0] <= p), np.all(p <= grid[i][-1])
        ):
            raise ValueError(
