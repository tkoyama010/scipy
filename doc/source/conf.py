...
inspect.isdescriptor = lambda obj: old_isdesc(obj) and not isinstance(obj, ua._Function)
...
warnings.filterwarnings(  # matplotlib<->pyparsing issue
    "ignore", message="Exception creating Regex for oneOf.*", category=SyntaxWarning
)
...
if os.path.isdir(themedir):
    ...
    if "scipyorg" in tags:
        raise RuntimeError(
            "Get the scipy-sphinx-theme first, " "via git submodule init & update"
        )
    else:
...
def linkcode_resolve(domain, info):
    ...
    if fn.startswith("scipy/"):
        ...
        if m:
            ...
            return "https://github.com/scipy/scipy/blob/master/%s%s" % (fn, linespec)
        else:
