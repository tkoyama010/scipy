...
class Lstsq(Benchmark):
    ...
    def setup(self, dtype, size, lapack_driver):
        ...
        if dtype is np.complex128:
            A = (10 * np.random.rand(m, k) - 5) + 1j * (10 * np.random.rand(m, k) - 5)
            temp = (10 * np.random.rand(k, n) - 5) + 1j * (
                10 * np.random.rand(k, n) - 5
            )
            b = (10 * np.random.rand(m, 1) - 5) + 1j * (10 * np.random.rand(m, 1) - 5)
        else:
