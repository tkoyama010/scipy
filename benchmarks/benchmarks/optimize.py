...
class _BenchOptimizers(Benchmark):
    ...
    def run_dualannealing(self):
        ...
        res = dual_annealing(self.fun, None, self.bounds)
...
class BenchGlobal(Benchmark):
    ...
    params = [list(_functions.keys()), ["success%", "<nfev>"], ["DE", "basinh.", "DA"]]
    ...
    def setup(self, name, ret_value, solver):
        if not self.enabled:
            print(
                "BenchGlobal.track_all not enabled --- export SCIPY_XSLOW=slow to enable,\n"
